...
class htmlParser {
	...
	private function addTag($id=0,$htmlText, $tag, $attr, $innerHtml, $singleTag=false) {
		
		if (!$singleTag || !$this->ignoreSingleTags) {
			$_id = $this->separator.$this->level.'-'.$id.$this->separator;
			$pairs=explode(" ",$attr);
			$attrarr=array();
			foreach($pairs as $strpair)
			{
				if (!empty($strpair))
				{
					list($key,$value)=explode("=",$strpair);
					$attarr[$key]=trim($value,"\"'");
				}
			}
			...
			if ($this->level>0) {
				if (strpos($innerHtml,$this->separator)!==false) { //has any unchanged tags in them
					...
					for ($i=0;$i<(count($ar)-1)/2;$i++) {
						...
						$this->idArray [$_id]['childNodes'] []= $element;
					...
					if ($singleTag) {
						$this->idArray [$_id]['htmlText'] = $innerHtml;
					} else {
						$this->idArray [$_id]['htmlText'] = '<'.$tag.' '.$attr.'>'.$innerHtml.'</'.$tag.'>';
					}
				}
			}
