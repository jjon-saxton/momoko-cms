...
interface MomokoResourceInterface
{
...
interface MomokoStorageInterface
{
	 public function __construct($path=null);
...
interface MomokoLITEPageObject
{
  public function toHTML($child=null);
}

interface MomokoLITEObject
{
...
class MomokoForm implements MomokoLITEObject
{
 public $formname;
 private $info=array();
 ...
 public function get()
 {
  $page=new MomokoLITEPage('/forms/'.$this->formname.'.htm');
  $this->info['title']=$page->title;
  $this->info['inner_body']=$page->inner_body;
  $this->info['full_html']=$page->full_html;
  return true;
 }
}
...
class MomokoCommentHandler
{
  ...
  private function loadMod($item,$argstr=null)
  {
    switch ($item)
    {
      ...
      case 'liteVersion':
      $vtxt=null;
      if (file_exists($GLOBALS['CFG']->basedir.'/assets/etc/version.nfo.txt'))
      {
       $vtxt.=$GLOBALS['CFG']->appname.' v'.file_get_contents($GLOBALS['CFG']->basedir.'/etc/version.txt');
      }
      if ($argstr == "copyright")
      {
       $mod=new MomokoLITEPage('/admin/copyright.htm');
       $vtxt.=$mod->inner_body;
      }
      trim($vtxt,"\r\n");
      return $vtxt;
