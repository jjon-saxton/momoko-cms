...
2. To download the latest stable version of MomoKO you need only type `wget --user=anonymous@momokocms.org ftp://momokocms.org/core/latest_stable/*.zip`. The '*' is a wildcard which will make wget grab all zip files in the directory '/core/latest_stable' There is only one archive in this folder at all times and it is the latest stable version of MomoKO so this will only grab the files you need. if you want a different version type `wget --user=anonymous@momokocms.org ftp://ftp.momokocms.org/core/momoko-cms-myversion.zip` replacing 'myversion' with the version you want. Packages are named with a major.minor version and a channel, for example 2.3-STABLE or 2.4-BETA. If you don't know what versions are available it's best to get a listing of the '/core/' directory using Linux's built in FTP client or a graphical one.
3. Extract the zip archive. This is a zip file so unzip will be needed to extract it. Type `unzip momoko-cms-2.3-STABLE.zip` or the file name you acquired from wget, you can also use `unzip momoko-cms*.zip` if you aren't sure. This will create a folder like 'momoko-cms-latest_stable' or 'momoko-cms-2.0-STABLE'. You may wish to simply rename this to 'momoko' or something similar.
4. Proceed to 'Configure MomoKO'

##### 1.2.2 Graphically: FileZilla

1. Open your favorite FTP client, for this example we'll be using FileZilla, it should be noted that, due to security concerns, our server does not accept anonymous connections in the standard sense. There is an account with limited access and no password that you will be using. This account is formatted in such a way that the built in clients for Windows and macOS do not work as expected. It is recommended to use a bonafide FTP client. Filezilla was chosen as it is free and available for Linux, Windows, and macOS.
2. Find the 'host' field and type `momokocms.org`. You may have to click connect or new site depending on your client.
4. Enter `anonymous@momokocms.org` as the username, but leave the password field blank. Click `quickconnect` or `Connect`. For your information connection can only be made throug the default FTP port, 21. Filezilla, and some other clients attempt to connect to FTP servers using TLS, please note, as of the writing of this document, momokocms.org does not have a valid certificate. You can continue to connect, but a warning may appear.
